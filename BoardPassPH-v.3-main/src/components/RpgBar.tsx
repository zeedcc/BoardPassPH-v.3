...
import { Flame, Zap, Percent, ShieldAlert, ShoppingBag, LogOut } from 'lucide-react';
...
export const RpgBar: React.FC<RpgBarProps> = ({ profile, setProfile, onNavigate, onLogout }) => {
  ...
  const buyStreakShield = () => {
    ...
    setShopMsg("🎉 Streak Shield Purchased! Your next missed daily study session is fully protected! 🛡️");
  };
    ...
    <div className="bg-gradient-to-r from-foam to-[#deebe3] border-b border-pine/10 px-4 py-3 shadow-sm select-none">
      ...
      {isShopOpen && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[2000] flex justify-center items-center p-4">
          <div className="bg-white max-w-md w-full rounded-2xl shadow-2xl p-6 relative overflow-hidden animate-in fade-in zoom-in-95 duration-150 border border-pine/10">
            ...
            <div className="border border-amber-100 bg-amber-50/40 rounded-xl p-4 flex flex-col gap-3 mb-4">
              <div className="flex justify-between items-center bg-white p-3 rounded-lg border border-amber-100 shadow-sm">
                ...
                <button
                  ...
                  className="px-3 py-1.5 text-xs font-sans font-black bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-600 text-white rounded-lg shadow disabled:opacity-40 select-none cursor-pointer"
                >
                  250 XP
                </button>
              </div>
