...
export const FeedbackPanel: React.FC<FeedbackPanelProps> = ({ profile }) => {
  ...
  const TOPICS = [
    ...
    { id: 'question', name: 'Diagnostic Question / Correction', emoji: '✏️' },
  ...
  const handleSubmitFeedback = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!message.trim()) {
      setErrorInfo('Please compose your feedback prior to submission!');
      return;
    }
      ...
      <div className="bg-gradient-to-br from-pine to-pine-mid rounded-2xl p-6 text-cream relative overflow-hidden select-none border border-pine-light/25">
        ...
        <div className="relative z-10 space-y-2">
          ...
          <p className="text-xs text-mint/80 max-w-xl leading-relaxed">
            Your scores, diagnostics, and reviews matter. Have a corrections query about a DSM-5 criteria, a bug report, or billing clarification? Our support team is here.
          </p>
        </div>
      </div>
            ...
            <div className="flex gap-2.5">
              ...
              <div>
                ...
                <p className="text-[10px] text-amber-800/80 leading-relaxed mt-1.5">
                  Our diagnostic modules and questions are engineered inside the Philippines. All references coordinate securely with the <strong>DSM-5-TR</strong> and international clinical standards.
                </p>
              </div>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmitFeedback} className="space-y-4">
            ...
            {successInfo && (
              <div className="space-y-3">
                <div className="bg-emerald-50 border border-emerald-100 text-emerald-800 p-3.5 rounded-xl flex gap-2.5 items-start animate-in fade-in duration-200">
                  <CheckCircle2 className="w-4 h-4 text-emerald-600 flex-shrink-0 mt-0.5 font-bold" />
                  <span className="text-[11px] leading-relaxed font-semibold">{successInfo}</span>
                </div>

                {lastSubmittedMsg && (
                  <div className="p-3.5 bg-emerald-50/50 border border-emerald-110 rounded-xl space-y-3 animate-in slide-in-from-top-2 duration-200">
                    <p className="text-[10px] uppercase font-bold text-[#1e3d29] tracking-wider font-mono">
                      ✉️ Swift Direct Send to Support:
                    </p>
            ...
            <div className="space-y-1">
              ...
              <input
                ...
                disabled
                value={profile.email}
                className="w-full bg-gray-50 border border-gray-150 pl-3 pr-3 py-2 text-xs font-semibold text-gray-550 rounded-xl cursor-not-allowed outline-none font-mono"
              />
            </div>
            ...
            <div className="space-y-1">
              ...
              <textarea
                ...
                className="w-full bg-white border border-pine/15 rounded-xl pl-3 pr-3 py-2 text-xs text-pine placeholder-pine-mid/30 outline-none focus:border-mint focus:ring-2 focus:ring-mint/10 leading-relaxed font-semibold resize-none"
              />
            </div>
