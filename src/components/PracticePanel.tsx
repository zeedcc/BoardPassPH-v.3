...
import { 
  ...
  ArrowRight, Upload, X, FileText, Trash2
} from 'lucide-react';
...
interface PracticePanelProps {
  ...
  loading: boolean;
  selectedModel: 'budget' | 'standard' | 'premium' | 'testbank';
  onModelChange: (model: 'budget' | 'standard' | 'premium' | 'testbank') => void;
}
...
export const PracticePanel: React.FC<PracticePanelProps> = ({
  ...
  const [uploadedFiles, setUploadedFiles] = useState<UploadedFileObj[]>(() => {
    try {
      ...
      if (saved) {
        const parsed = JSON.parse(saved) as UploadedFileObj[];
        if (Array.isArray(parsed)) {
          // Rehydrate base64 from sessionStorage if present to keep heavy files out of localStorage
          return parsed.map(f => {
            try {
              const cachedBase64 = sessionStorage.getItem(`bp_file_base64_${f.id}`);
              return cachedBase64 ? { ...f, base64: cachedBase64 } : f;
            } catch {
              return f;
            }
          });
  ...
  const safeSaveUploadedFiles = (files: UploadedFileObj[]) => {
    try {
      // 1. Save actual heavy base64 data to sessionStorage, which has high quotas and is tab-lifecycle isolated
      files.forEach(f => {
        if (f.base64) {
          try {
            sessionStorage.setItem(`bp_file_base64_${f.id}`, f.base64);
          } catch (sessionErr) {
            console.warn("Could not write file base64 to sessionStorage:", sessionErr);
          }
        }
      });

      // 2. Map and serialize ONLY the metadata to localStorage to prevent QuotaExceededError crashes completely!
      const metadataOnly = files.map(({ base64, ...rest }) => rest);
      localStorage.setItem('bp_uploaded_files', JSON.stringify(metadataOnly));
    } catch (err) {
      console.warn("Could not save uploaded file reference metadata to localStorage:", err);
    }
  };
  ...
  const handleFetchNext = () => {
    if (atDailyLimit) {
      alert(`Daily AI question limit reached (${dailyLimit}/day on ${profile.tier}). Upgrade in Plans for unlimited practice.`);
      return;
    }
    setSelectedIdx(null);
    ...
    if (selectedFileId && activeFile && !activeFile.base64) {
      alert(`⚠️ To generate custom AI questions from "${activeFile.name}", please click on the Browse/Drag-and-Drop section to quickly re-upload it. This session-only approach preserves browser storage limits and prevents crashes!`);
      return;
    }

    let finalDifficulty = difficulty;
    if (profile.adaptive) {
      if (profile.currentCombo < 1) {
        finalDifficulty = 'easy';
      } else if (profile.currentCombo >= 1 && profile.currentCombo < 3) {
  ...
  const handleFiles = (files: FileList) => {
    ...
    if (file.size > 10 * 1024 * 1024) {
      alert("⚠️ File size exceeds the maximum limit of 10MB. Please upload a smaller study resource.");
      return;
    }
    ...
    reader.onload = () => {
      try {
        ...
        setUploadedFiles(prev => {
          ...
          // Execute safe persist outside of the state transaction to be pure and prevent commit phase exceptions
          setTimeout(() => {
            safeSaveUploadedFiles(next);
          }, 0);
          return next;
        });
        ...
        alert("❌ Error processing the file. Please try again with another file.");
      }
    };
  ...
  const deleteFile = (id: string, e: React.MouseEvent) => {
    e.stopPropagation();
    try {
      sessionStorage.removeItem(`bp_file_base64_${id}`);
    } catch {}
    setUploadedFiles(prev => {
    ...
    setProfile(prev => {
      ...
      if (prev.rememberQuestionHistory !== false && currentQuestion?.vignette) {
        ...
        const historyEntry = JSON.stringify({
          vignette: currentQuestion.vignette.substring(0, 100),
          subject,
          correct: isCorrect,
          timestamp: new Date().toISOString(),
          topic: currentQuestion.topic || currentQuestion.category
        });
        const existingHistory = prev.questionHistory || [];
        const normQ = currentQuestion.vignette.replace(/[^a-zA-Z0-9]/g, '').toLowerCase().substring(0, 80);
        const duplicate = existingHistory.some(item => {
          if (typeof item !== 'string') return false;
          try {
            let itemVig = item;
            if (item.trim().startsWith('{')) {
              const parsed = JSON.parse(item);
              if (currentQuestion.topic && parsed.topic && currentQuestion.topic.toLowerCase().trim() === parsed.topic.toLowerCase().trim()) {
                return true;
              }
              itemVig = parsed.vignette || '';
            }
            const normItem = itemVig.replace(/[^a-zA-Z0-9]/g, '').toLowerCase().substring(0, 80);
            return normItem.includes(normQ) || normQ.includes(normItem);
          } catch {
            const normItem = item.replace(/[^a-zA-Z0-9]/g, '').toLowerCase().substring(0, 80);
            return normItem.includes(normQ) || normQ.includes(normItem);
          }
        });
        if (!duplicate) {
          profilePayload.questionHistory = [...existingHistory, historyEntry].slice(-100);
        } else {
          profilePayload.questionHistory = existingHistory;
        }
      }
      ...
      <div className="bg-white border border-gray-200 rounded-2xl p-5 shadow-sm space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="space-y-1 md:border-l border-gray-100 md:pl-4">
            ...
            <select
              disabled={profile.adaptive}
              value={profile.adaptive ? (profile.currentCombo < 1 ? 'easy' : profile.currentCombo < 3 ? 'medium' : 'hard') : difficulty}
              onChange={(e) => setDifficulty(e.target.value as any)}
              className={`w-full bg-white border border-gray-200 text-xs font-semibold py-2.5 px-3 rounded-xl outline-none focus:border-sage transition-all ${
                profile.adaptive ? 'opacity-65 cursor-not-allowed bg-gray-50 text-gray-500' : ''
              }`}
            >
            ...
            {profile.adaptive && (
              <p className="text-[9px] text-[#2e5e41] font-mono font-bold mt-1 inline-flex items-center gap-1">
                <span>⚡ ADAPTIVE OVERRIDE:</span>
                <span className="uppercase bg-[#deebe3] px-1.5 py-0.5 rounded text-[8px] font-black">
                  {profile.currentCombo < 1 ? 'easy' : profile.currentCombo < 3 ? 'medium' : 'hard'}
                </span>
                <span>(Combo: {profile.currentCombo})</span>
              </p>
            )}
          </div>
        ...
        <div className="border-t border-gray-100 pt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="space-y-1.5">
            ...
            <select
              value={selectedModel}
              onChange={(e) => onModelChange(e.target.value as 'budget' | 'standard' | 'premium' | 'testbank')}
              className="w-full bg-white border border-gray-200 text-xs font-semibold py-2.5 px-3 rounded-xl outline-none focus:border-sage transition-all"
            >
              ...
              <option value="testbank">📦 Offline Board Test Bank — random high-yield seed questions</option>
            </select>
          </div>

          <div className="flex items-center justify-between bg-foam/15 border border-pine/5 p-3 rounded-xl">
            <div className="space-y-0.5">
              <span className="text-[10px] uppercase font-bold text-teal-800 block tracking-wider flex items-center gap-1 font-mono">
                ⚡ Adaptive Testing Engine
              </span>
              <p className="text-[10px] text-gray-400 leading-tight">
                Dynamically adjusts difficulty based on your streak. (0 combo = Easy, 1-2 = Medium, 3+ = Hard)
              </p>
            </div>
            <label className="relative inline-flex items-center cursor-pointer select-none shrink-0">
              <input 
                ...
                onChange={(e) => {
                  setProfile(prev => {
                    const updated = { ...prev, adaptive: e.target.checked };
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-3">
            <div className="lg:col-span-5">
              ...
              >
                ...
                <span className="text-[8px] text-gray-400 font-mono mt-0.5">Supports PDF, TXT, PNG, JPG (Max 10MB)</span>
              </div>
            </div>
          ...
          <div className="bg-white border border-gray-200 border-l-4 border-l-pine-light rounded-2xl p-5 shadow-sm space-y-4">
            ...
            <div className="pt-3 border-t border-dashed border-gray-100 space-y-1">
              ...
              <textarea
                ...
                rows={2}
                className="w-full bg-foam/40 border border-pine/10 focus:border-sage pl-3 pr-3 py-2 text-xs font-sans rounded-xl outline-none resize-none transition-all placeholder:text-gray-400 text-pine font-medium"
              />
            </div>
          </div>
            ...
            <div className="bg-gradient-to-br from-foam to-[#e4eed9] border border-[#cbd9bc] rounded-2xl p-5 space-y-4 shadow-inner animate-in slide-in-from-top-4 duration-150">
              ...
              {(loadingMnemonic || mnemonicText) && (
                <div className="border border-amber-200 bg-white/70 p-4 rounded-xl shadow-inner space-y-2 animate-in fade-in duration-100">
                  ...
                  {!loadingMnemonic && mnemonicText && (
                    <div className="text-[11px] text-gray-700 leading-relaxed font-sans space-y-1">
                      {(() => {
                        ...
                        // Group lines into blocks: 'table' | 'other'
                        type Block = { type: 'table'; rows: string[] } | { type: 'other'; line: string; idx: number };
                        const blocks: Block[] = [];
                        ...
                        while (i < lines.length) {
                          ...
                          if (t.startsWith('|') && t.endsWith('|')) {
                            ...
                            blocks.push({ type: 'other', line: lines[i], idx: i });
                            i++;
                          }
                        }
                        ...
                        return blocks.map((block, bi) => {
                          if (block.type === 'table') {
                            // Parse markdown table rows into cell arrays, skip separator rows (|---|)
                            ...
                            return (
                              <div key={bi} className="overflow-x-auto my-2 rounded-xl border border-amber-200 shadow-sm">
                                <table className="w-full text-[10.5px] border-collapse">
                                  <thead>
                                    <tr className="bg-amber-50">
                                      {head.map((cell, ci) => (
                                        <th key={ci} className="px-3 py-2 text-left font-black text-amber-900 border-b border-amber-200 whitespace-nowrap">
                                          {renderInline(cell)}
                                        </th>
                                      ))}
                                    </tr>
                                  ...
                                  <tbody>
                                    {body.map((row, ri) => (
                                      <tr key={ri} className={ri % 2 === 0 ? 'bg-white' : 'bg-amber-50/40'}>
                                        {row.map((cell, ci) => (
                                          <td key={ci} className="px-3 py-2 border-b border-amber-100 align-top">
                                            {renderInline(cell)}
                                          </td>
                                        ))}
                                      </tr>
                          ...
                          const { line, idx } = block as { type: 'other'; line: string; idx: number };
                          ...
                          if (trimmed.startsWith('*   ') || trimmed.startsWith('* ')) {
                            ...
                            return (
                              <div key={bi} className="flex items-start gap-2 pl-1">
                                ...
                                <span className="font-medium">{renderInline(content)}</span>
                              </div>
                            );
                          }
                          if (trimmed.startsWith('*') && trimmed.endsWith('*') && !trimmed.startsWith('**')) {
                            return <p key={bi} className="text-[10px] italic text-gray-500 mt-1 pl-1">{trimmed.slice(1, -1)}</p>;
                          }
                          if (trimmed === '') return <div key={bi} className="h-1" />;
                          return <p key={bi} className="font-medium">{renderInline(line)}</p>;
                        });
                      })()}
                    </div>
                  )}
